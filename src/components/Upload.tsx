...
import { activeSnapshotStore, monthArchiveStore, newId } from "@/lib/store";
...
export function Upload({ onDone }: { onDone: (snap: UploadSnapshot) => void }) {
  ...
  async function run() {
    ...
    try {
      ...
      clearSessionCache(); // New upload = invalidate all cached AI results.
      await activeSnapshotStore.set(snap);
      // Archive per-month KPI totals — the only cross-upload persistence.
      const { toViewPosts } = await import("@/lib/view-mode");
      const { monthlyKpiSnapshots } = await import("@/lib/deltas");
      const monthSnaps = monthlyKpiSnapshots(toViewPosts(result.posts, "total"));
      const archives: KpiSnapshot[] = monthSnaps.map(({ monthKey, monthLabel, totals, posts }) => ({
        monthKey,
        monthLabel,
        reach: totals.reach,
      ...
      if (archives.length) await monthArchiveStore.upsertMany(archives);
      onDone(snap);
    } catch (e: any) {
