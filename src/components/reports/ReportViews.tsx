...
import { AiAnalyzing, AiUsageSection } from "@/components/ai/AiControls";
...
function NeedsKey() {
  return (
    <Empty
      ...
      body="The quantitative summary above is always live. The strategist analysis is generated automatically when a GEMINI_API_KEY is configured; otherwise this report stays quantitative-only."
    />
  );
}
...
export function MonthlyReport({ posts }: { posts: ConsolidatedPost[] }) {
  ...
  return (
    <div className="fade-in">
      ...
      />
      <MiniKpis posts={posts} />
      <div className="mt-6"><AiAnalyzing /></div>
...
function dailyToSections(r: AiReport): AiNarrativeSection[] {
  ...
  const st = r.strategist;
  if (st?.executiveSummary) s.push({ heading: "Executive Summary", lines: [st.executiveSummary] });
  if (st?.whereToActNext?.length)
    s.push({ heading: "Where to Act Next", lines: st.whereToActNext.map((f) => `[${f.priority}] ${f.title} — ${f.recommendedAction} (expected: ${f.expectedOutcome})`) });
  if (st?.risks?.length) s.push({ heading: "Risks & Watchouts", lines: st.risks.map((f) => `${f.title} — ${f.whatHappened}`) });
  if (st?.growthLevers?.length) s.push({ heading: "Growth Levers", lines: st.growthLevers.map((f) => `${f.title} — ${f.recommendedAction}`) });
  if (st?.opportunities?.length) s.push({ heading: "Opportunities", lines: st.opportunities.map((o) => `${o.title} — ${o.recommendedAction}`) });
  return s;
}
