...
function useRecommendations() {
  ...
  useEffect(() => {
    let alive = true;
    const cacheKey = "recommendations:daily";
    const cached = getSessionCache<AiRecommendationsResponse>(cacheKey);
    if (cached) {
      setState(cached.enabled ? { status: "ready", data: cached } : { status: "disabled", data: cached, error: cached.reason });
      return;
    }
    setState({ status: "loading" });
    fetchWithDedup<AiRecommendationsResponse>(cacheKey, () =>
  ...
  return (
    <div>
      <SectionTitle
        ...
        title="30 Brand-Level Content Recommendations"
        hint="Fifteen flagship recurring formats plus fifteen trend-anchored one-offs. Every recommendation names brands from the TCL catalog with a per-brand rationale, drafts a ready-to-publish caption in the @tatacliqluxury voice, and ties to a live cultural signal. Refreshed once per calendar day."
      />
      ...
      {status === "loading" && (
        <Card className="p-6">
          <div className="flex items-center gap-2 text-[13px] text-slate">
            ...
            Drafting 30 in-voice recommendations across 2 batches — reading @tatacliqluxury exemplars, scanning Vogue, BoF, WWD, Google Trends, celebrity news, festivals…
          </div>
        </Card>
      )}
