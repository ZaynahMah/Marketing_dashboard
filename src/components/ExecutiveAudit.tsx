...
import { executiveSummary, recommendations, strategicInsights } from "@/lib/insights";
...
export function ExecutiveAudit({ posts, label }: { posts: ConsolidatedPost[]; label: string }) {
  ...
  const recs = recommendations(posts);
    ...
    <div className="fade-in">
      ...
      <div className="mt-8">
        <SectionTitle
          ...
          hint="Auto-generated from the consolidated dataset. Every line is backed by the numbers below."
        />
      ...
      {/* Strategic insights */}
      <div className="mt-12">
        <SectionTitle eyebrow="Strategic Insights" title="Wins, risks & opportunities" />
      ...
      <div className="mt-12">
        <SectionTitle
          ...
          hint="Prioritised for the CMO. Each recommendation ties observed performance to the Tata CLiQ Luxury content philosophy."
        />
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="flex-1">
                  <div className="mb-2 flex items-center gap-2">
                    <Pill tone={r.priority === "High" ? "risk" : "default"}>{r.priority} priority</Pill>
                  </div>
                  <h3 className="display text-[17px] font-medium leading-snug text-ink">{r.headline}</h3>
                  <p className="mt-2 text-[13px] leading-relaxed text-graphite">{r.rationale}</p>
                  <p className="mt-2 text-[12px] italic leading-relaxed text-claret">{r.brandAlignment}</p>
                </div>
                <div className="w-full shrink-0 border-t border-hairline pt-3 sm:w-56 sm:border-l sm:border-t-0 sm:pl-4 sm:pt-0">
                  <Eyebrow>Evidence</Eyebrow>
                  <p className="tabular mt-2 text-[12px] leading-relaxed text-slate">{r.evidence}</p>
                </div>
              </div>
            </Card>
          ))}
