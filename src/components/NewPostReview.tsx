...
export function NewPostReview({ posts }: Props) {
  ...
  // Auto-trigger when we have at least one image.
  useEffect(() => {
    if (!previews.length) return;
    // Debounce: wait 600ms after last change before calling.
    const timer = setTimeout(() => {
      ...
      fetch("/api/review", {
        ...
        .then((data: ReviewResponse) => {
          if (!data.enabled) return setStatus("error");
          setResult(data);
          setStatus("ready");
        })
  ...
  }, [previews, caption, brand, histCtx]);

  const r = result?.review as Record<string, unknown> | undefined;
  ...
  return (
    <div className="fade-in">
      ...
      {status === "error" && <Empty title="Review unavailable" body={result?.reason ?? "Check GEMINI_API_KEY is set in Vercel."} />}
          ...
          {result?.usage && (
            <div className="border-t border-hairline pt-5">
              ...
              <p className="tabular text-[11px] text-mist">
                {result.model} · Input {result.usage.inputTokens.toLocaleString()} · Output {result.usage.outputTokens.toLocaleString()} · Total {result.usage.totalTokens.toLocaleString()}
                {result.usage.estimatedCostUsd != null && ` · Est. cost $${result.usage.estimatedCostUsd.toFixed(4)}`}
              </p>
            </div>
          )}
        </div>
