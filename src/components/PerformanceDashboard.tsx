...
import { Card, Empty, Eyebrow, Pill, SectionTitle } from "./ui";
...
import { monthBands, weekBands } from "@/lib/deltas";
...
const SECTIONS = ["KPIs", "Trends", "Content", "Format", "Rankings", "Posts", "Analysis", "Recommendations", "New Post", "Competitors"] as const;
type Section = (typeof SECTIONS)[number];

const MODE_LABELS: Record<ViewMode, string> = { total: "Total", paid: "Paid", organic: "Organic" };

export function PerformanceDashboard({ posts, archive = [], snapshotId = "current" }: { posts: ConsolidatedPost[]; archive?: KpiSnapshot[]; snapshotId?: string }) {
  ...
  return (
    <div className="fade-in">
      ...
      <div className="mt-6">
        {active === "KPIs" && <KpisPane posts={viewPosts} archive={archive} mode={mode} />}
        ...
        {active === "Posts" && <PostTable posts={posts} snapshotId={snapshotId} mode={mode} />}
        {active === "Analysis" && <AnalysisPanel snapshotId={snapshotId} posts={posts} mode={mode} />}
        ...
        {active === "New Post" && <NewPostReview posts={posts} />}
...
/* ------------------------- KPI PANE ------------------------- */
function KpisPane({ posts, archive, mode }: { posts: ViewPost[]; archive: KpiSnapshot[]; mode: ViewMode }) {
  ...
  // Archive fallback for MoM when the active dataset only spans one month.
  // Find the archived month immediately preceding whatever month the active dataset represents.
  const archivePrev = useMemo(() => {
    if (prevMonthT) return null;
    if (!archive.length) return null;
    const currentLabel = monthly.currentLabel;
    if (currentLabel === "—") return archive[archive.length - 1] ?? null;
    const idx = archive.findIndex((a) => a.monthLabel === currentLabel);
    if (idx > 0) return archive[idx - 1];
    return archive[archive.length - 1] ?? null;
  }, [prevMonthT, archive, monthly.currentLabel]);
  ...
  const rows: {
    ...
    { label: "Reach", value: fmtCompact(t.reach), curNum: t.reach, prevMonth: prevMonthT?.reach ?? archivePrev?.reach ?? null, prevWeek: prevWeekT?.reach ?? null },
    { label: "Views", value: fmtCompact(t.views), curNum: t.views, prevMonth: prevMonthT?.views ?? archivePrev?.views ?? null, prevWeek: prevWeekT?.views ?? null },
    { label: "Impressions", value: t.impressions ? fmtCompact(t.impressions) : "—", curNum: t.impressions || null, prevMonth: prevMonthT?.impressions ?? archivePrev?.impressions ?? null, prevWeek: prevWeekT?.impressions ?? null, unavailable: mode === "organic" ? "paid-only" : undefined },
    { label: "Engagement", value: fmtCompact(t.interactions), curNum: t.interactions, prevMonth: prevMonthT?.interactions ?? archivePrev?.interactions ?? null, prevWeek: prevWeekT?.interactions ?? null },
    { label: "Likes", value: t.likes ? fmtCompact(t.likes) : "—", curNum: t.likes || null, prevMonth: prevMonthT?.likes ?? archivePrev?.likes ?? null, prevWeek: prevWeekT?.likes ?? null, unavailable: mode === "paid" ? "aggregated in Engagement" : undefined },
    { label: "Saves", value: fmtInt(t.saves), curNum: t.saves, prevMonth: prevMonthT?.saves ?? archivePrev?.saves ?? null, prevWeek: prevWeekT?.saves ?? null },
    { label: "Shares", value: fmtInt(t.shares), curNum: t.shares, prevMonth: prevMonthT?.shares ?? archivePrev?.shares ?? null, prevWeek: prevWeekT?.shares ?? null },
    { label: "Comments", value: fmtInt(t.comments), curNum: t.comments, prevMonth: prevMonthT?.comments ?? archivePrev?.comments ?? null, prevWeek: prevWeekT?.comments ?? null },
    { label: "Reposts", value: "—", curNum: null, prevMonth: null, prevWeek: null, unavailable: "not in IG export" },
    { label: "Profile Activity", value: fmtInt(t.profileVisits), curNum: t.profileVisits, prevMonth: prevMonthT?.profileVisits ?? null, prevWeek: prevWeekT?.profileVisits ?? null },
    { label: "Follows (from posts)", value: fmtInt(t.follows), curNum: t.follows, prevMonth: prevMonthT?.follows ?? archivePrev?.follows ?? null, prevWeek: prevWeekT?.follows ?? null },
    { label: "Gross Followers", value: "—", curNum: null, prevMonth: null, prevWeek: null, unavailable: "needs account overview export" },
    { label: "Net Followers", value: "—", curNum: null, prevMonth: null, prevWeek: null, unavailable: "needs account overview export" },
    { label: "Increase in Followers", value: "—", curNum: null, prevMonth: null, prevWeek: null, unavailable: "needs account overview export" },
    { label: "Actual Spend", value: fmtCurrency(t.spend), curNum: t.spend, prevMonth: prevMonthT?.spend ?? archivePrev?.spend ?? null, prevWeek: prevWeekT?.spend ?? null },
    { label: "Spend (Lakhs)", value: `₹${t.spendLakhs.toFixed(2)}L`, curNum: t.spendLakhs, prevMonth: prevMonthT?.spendLakhs ?? (archivePrev ? archivePrev.spend / 100000 : null), prevWeek: prevWeekT?.spendLakhs ?? null },
    { label: "ER%", value: fmtPct(t.er, 3), curNum: t.er, prevMonth: prevMonthT?.er ?? archivePrev?.er ?? null, prevWeek: prevWeekT?.er ?? null },
    { label: "CPE", value: fmtCost(t.cpe), curNum: t.cpe, prevMonth: prevMonthT?.cpe ?? archivePrev?.cpe ?? null, prevWeek: prevWeekT?.cpe ?? null, lowerIsBetter: true },
    { label: "CPR", value: fmtCost(t.cpr), curNum: t.cpr, prevMonth: prevMonthT?.cpr ?? archivePrev?.cpr ?? null, prevWeek: prevWeekT?.cpr ?? null, lowerIsBetter: true },
    { label: "CPV", value: fmtCost(t.cpv), curNum: t.cpv, prevMonth: prevMonthT?.cpv ?? archivePrev?.cpv ?? null, prevWeek: prevWeekT?.cpv ?? null, lowerIsBetter: true },
    { label: "CPF", value: fmtCost(t.cpf), curNum: t.cpf, prevMonth: prevMonthT?.cpf ?? archivePrev?.cpf ?? null, prevWeek: prevWeekT?.cpf ?? null, lowerIsBetter: true },
    { label: "CTR", value: fmtPct(t.ctr, 3), curNum: t.ctr, prevMonth: prevMonthT?.ctr ?? null, prevWeek: prevWeekT?.ctr ?? null },
    { label: "Avg Watch Time", value: t.avgWatchSeconds != null ? `${t.avgWatchSeconds.toFixed(1)}s` : "—", curNum: t.avgWatchSeconds, prevMonth: prevMonthT?.avgWatchSeconds ?? null, prevWeek: prevWeekT?.avgWatchSeconds ?? null },
    { label: "Skip Rate", value: t.skipRate != null ? `${t.skipRate.toFixed(1)}%` : "—", curNum: t.skipRate, prevMonth: prevMonthT?.skipRate ?? null, prevWeek: prevWeekT?.skipRate ?? null, lowerIsBetter: true },
  ];
  ...
  return (
    <section>
      <SectionTitle
        ...
        hint="Month-over-month uses the archive when the active dataset only spans one month. Trend arrows follow whether higher or lower is better."
      />
...
function RankingsPane({ posts }: { posts: ViewPost[] }) {
  const groups: { title: string; by: RankBy; extract: (p: ViewPost) => string }[] = [
    ...
    { title: "Lowest CPF", by: "cpf", extract: (p) => (p.spend && p.follows ? fmtCost(p.spend / p.follows) : "—") },
  ];
  return (
    <section>
      <SectionTitle
        eyebrow="Rankings"
        title="Top performers by nine criteria"
        hint="Engagement, discovery, distribution, and the four cost efficiencies — CPE, CPV, CPR, CPF — surfaced side by side."
      />
