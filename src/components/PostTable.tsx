...
import { fetchWithDedup, getSessionCache } from "@/lib/session-cache";
...
const COLS: Col[] = [
  ...
  { key: "cpf", label: "CPF", align: "right", extract: (p) => {
      const s = p.spend ?? 0;
      const f = p.follows ?? 0;
      return s > 0 && f > 0 ? s / f : null;
    }, sortValue: (p) => {
      const s = p.spend ?? 0;
      const f = p.follows ?? 0;
      return s > 0 && f > 0 ? s / f : Number.POSITIVE_INFINITY;
    } },
...
const FORMATTERS: Record<string, (v: any) => string> = {
  ...
  cpe: fmtCost, cpr: fmtCost, cpv: fmtCost, cpf: fmtCost, cpm: fmtCost,
};
...
export function PostTable({ posts, snapshotId, mode = "total" }: { posts: ConsolidatedPost[]; snapshotId?: string; mode?: ViewMode }) {
  ...
  const [verdicts, setVerdicts] = useState<Map<string, AiPostAnalysis>>(new Map());

  // Fetch verdicts from the analysis endpoint — uses session cache so tab switch is instant.
  useEffect(() => {
    if (!snapshotId) return;
    let alive = true;
    const view = toViewPosts(posts, mode);
    const medians = computeMedians(view);
    // Prime with deterministic verdicts immediately.
    const primed = new Map<string, AiPostAnalysis>();
    for (const p of view) primed.set(p.shortcode, deterministicPostReport(p, medians));
    setVerdicts(primed);

    const cacheKey = `analysis:${snapshotId}-${mode}`;
    const cached = getSessionCache<AiAnalysisResponse>(cacheKey);
    if (cached?.enabled) {
      const merged = new Map(primed);
      (cached.postAnalyses ?? []).forEach((a) => { if (a.shortcode) merged.set(a.shortcode, a); });
      setVerdicts(merged);
      return;
    }

    fetchWithDedup<AiAnalysisResponse>(cacheKey, () =>
      fetch("/api/analysis", {
        method: "POST",
      ...
      .then((data) => {
        ...
        const merged = new Map(primed);
        (data.postAnalyses ?? []).forEach((a) => { if (a.shortcode) merged.set(a.shortcode, a); });
        setVerdicts(merged);
      })
      .catch(() => void 0);
    return () => { alive = false; };
  }, [snapshotId, posts, mode]);
        ...
        <table className="min-w-full text-left">
          <thead className="sticky top-0 bg-surface">
            <tr className="border-b border-line">
              ...
              {snapshotId && (
                <th className="whitespace-nowrap px-3 py-2.5 text-left"><Eyebrow>AI Verdict</Eyebrow></th>
              )}
            </tr>
          </thead>
          <tbody>
              ...
              <tr key={p.shortcode + i} className="border-b border-hairline last:border-0">
                ...
                {snapshotId && (
                  <td className="whitespace-nowrap px-3 py-2 text-[12px]">
                    {(() => {
                      const v = verdicts.get(p.shortcode);
                      if (!v) return <span className="text-mist">…</span>;
                      return (
                        <div className="flex items-center gap-1.5">
                          <Pill tone={VERDICT_TONE[v.verdict] ?? "default"}>{v.verdict}</Pill>
                          <span className="tabular text-[10px] text-mist">{v.confidenceScore}%</span>
                        </div>
                      );
                    })()}
                  </td>
