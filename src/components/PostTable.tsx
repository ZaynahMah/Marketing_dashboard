...
export function PostTable({ posts, snapshotId, mode = "total" }: { posts: ConsolidatedPost[]; snapshotId?: string; mode?: ViewMode }) {
  ...
  // Fetch verdicts from the analysis endpoint (same cache as the Analysis tab).
  useEffect(() => {
    if (!snapshotId) return;
    let alive = true;
    const view = toViewPosts(posts, mode);
    const medians = computeMedians(view);
    // Prime the map immediately with deterministic verdicts so nothing is blank while AI loads.
    ...
    fetch("/api/analysis", {
      ...
      .then((data: AiAnalysisResponse) => {
        ...
        // Overlay AI verdicts on top of the deterministic base — AI wins where present.
        const merged = new Map(primed);
        (data.postAnalyses ?? []).forEach((a) => {
          if (a.shortcode) merged.set(a.shortcode, a);
        });
        setVerdicts(merged);
      })
      .catch(() => void 0);
