...
export function AnalysisPanel({ snapshotId, posts, mode }: Props) {
  ...
  useEffect(() => {
    ...
    if (cached) {
      setState(cached.enabled ? { status: "ready", data: cached } : { status: "disabled", data: cached, error: cached.reason });
      return;
    }
              ...
              <Card key={p.shortcode} className="p-5">
                ...
                {open && a && (
                  <div className="mt-4 grid gap-3 border-t border-hairline pt-4 md:grid-cols-2">
                    ...
                    {a.trendAlignment && <ReadRow label="Trend alignment" body={a.trendAlignment} tone="accent" />}
                    {a.creativeDifferentiation && <ReadRow label="Creative differentiation" body={a.creativeDifferentiation} />}
                    {a.storytellingQuality && <ReadRow label="Storytelling quality" body={a.storytellingQuality} />}
                    {a.captionEffectiveness && <ReadRow label="Caption effectiveness" body={a.captionEffectiveness} />}
                    {a.brandPositioning && <ReadRow label="Brand positioning" body={a.brandPositioning} />}
                    {a.visualNarrative && <ReadRow label="Visual narrative" body={a.visualNarrative} />}
                    {a.ctaQuality && <ReadRow label="CTA quality" body={a.ctaQuality} />}
                    {a.timingAndCulturalRelevance && <ReadRow label="Timing & cultural relevance" body={a.timingAndCulturalRelevance} tone="accent" />}
                    ...
                    {a.whatToRepeat && <ReadRow label="What to repeat" body={a.whatToRepeat} tone="win" />}
                    {a.whatToImprove && <ReadRow label="What to improve" body={a.whatToImprove} tone="accent" />}
                    {a.whatToAvoid && <ReadRow label="What to avoid" body={a.whatToAvoid} tone="risk" />}
                  </div>
                )}
              </Card>
            );
