...
export function AnalysisPanel({ snapshotId, posts, mode }: Props) {
  ...
  const byShortcode = useMemo(() => {
    const map = new Map<string, AiPostAnalysis>();
    (state.data?.postAnalyses ?? []).forEach((a) => map.set(a.shortcode, a));
    return map;
  }, [state.data]);
    ...
    <div className="fade-in">
      ...
      {state.status === "ready" && (
        <div className="space-y-3">
          {view.map((p) => {
            const a = byShortcode.get(p.shortcode);
            ...
            return (
              <Card key={p.shortcode} className="p-5">
                ...
                {open && !a && <p className="mt-3 border-t border-hairline pt-3 text-[12px] text-mist">No analysis returned for this post.</p>}
              </Card>
            );
          })}
        </div>
