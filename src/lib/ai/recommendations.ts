...
const TOTAL_RECS = 50;
const FLAGSHIP_COUNT = 10;
const BATCH_SIZE = 17; // 3 batches ≈ 17+17+16 = 50; keeps each call comfortably within token budget
...
export async function callRecommendations(model: AiModelId, apiKey: string, dateStr: string): Promise<RecsCallResult> {
  ...
  const batchSizes = [BATCH_SIZE, BATCH_SIZE, TOTAL_RECS - 2 * BATCH_SIZE];
  const flagshipQuotas = [4, 3, 3];
