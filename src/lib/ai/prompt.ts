...
Reject on sight any phrase that could apply to any brand. Do not write:
- "Post more Reels" / "Increase engagement" / "Reach was low" / "Views were down"
- "Leverage" / "elevate" / "curated" / "authentic storytelling" / "meaningful content"
- Recommendations that only name a format ("try Reels") without also naming a specific content theme, brand, and reference
- Restatements of the number without a WHY (e.g. "ER was 1.1%, so ER is low" — no)
- Any advice that a competitor's team could also receive verbatim

=== SPECIFICITY BAR (every recommendation must have all four) ===
1. EXACT content theme — name it (e.g. "Longines Master Collection heritage montage tied to the World Chess Championship")
2. EXACT brand from the TCL catalog — use one or two named brands per recommendation, never "luxury brands" or "watch brands"
3. EXACT storytelling style — name the register (behind-the-atelier, celebrity-on-set, connoisseur-testimonial, editorial-still-life, ambient-ASMR-craft, etc.)
4. EXACT execution — one sentence on shot list, editorial reference, or cinematography

Every WHY must ground in at least ONE of: (a) a specific cited number from the supplied data, (b) a current named trend/moment (name the moment), (c) a named competitor move, (d) a specific brand register from the catalog.

Every whatWorked / whatDidntWork item must give 2-3 DISTINCT creative reasons — hook design, cultural timing, product relevance, format-fit, celebrity or influencer resonance, audience behaviour, cost efficiency, competitive whitespace — never the same reason twice.

Every insight must answer at least one of: "hidden opportunity", "missed pattern", "market signal", "creative change we should make now".`;
...
const STRATEGIST_JSON = `"strategist": {
  ...
  "consulting": {
    "keyWins": [{"point":"one specific win in ONE sentence","metric":"the exact supporting number, e.g. 'ER 3.38% vs 1.11% organic-only baseline'"}],
    "redFlags": [{"point":"one specific weakness in ONE sentence","metric":"the exact supporting number"}],
    "contentGaps": {
      "trendsNotLeveraged": [{"point":"a NAMED luxury trend the brand did not engage (Quiet Luxury, Investment Dressing, Craftsmanship, Slow Fashion, Sustainable Luxury, Men's Luxury, Inside Atelier, Luxury Resale, etc.)","metric":"why this shows up as a gap in the data — e.g. '0 posts under Craftsmanship bucket, yet Craftsmanship posts elsewhere generate 3x more saves'"}],
      "formatsNotExplored": [{"point":"a NAMED format the brand did not use (Instagram Live, Brand Films, POV Reels, Countdown Drops, Guides, Unboxing, ASMR craft, Designer Conversations, etc.)","metric":"data-supported gap"}],
      "audienceSignalsIgnored": [{"point":"a NAMED audience signal ignored (Gift Guides, Wedding Luxury, Founder Stories, Collector Stories, Watch Collectors, Tier 2 Aspirational, Styling Advice, Luxury Education, etc.)","metric":"data-supported gap"}]
    },
    "audienceInsights": {
      "saveShareBehaviour": "one plain sentence citing save/share numbers",
      "contentPreferenceSignals": "which buckets/formats the audience prefers, per numbers",
      "audienceIntent": "consideration, discovery, transactional — inferred from CTR/saves/spend numbers",
      "timingInsights": "day-of-week / time-of-day patterns you can infer from the dates",
      "postingFrequencyInsights": "cadence read — under-posting or over-posting, per posts-per-day math",
      "optimalPostingCadence": "the recommended cadence with a data reason"
    },
    "brutalTruth": "ONE consulting paragraph (McKinsey / Bain / BCG voice meets luxury brand strategy). No sugarcoating. State what is fundamentally wrong, what must change, why, what happens if nothing changes, and what to prioritise. 5-7 sentences maximum."
  }
}
...
Provide 2-3 findings per list (whereToActNext 3). For the consulting section: keyWins 3-5, redFlags 3-5, each contentGaps sub-list 3-4. Keep EVERY field to one tight, specific sentence. Every 'metric' MUST reference an actual number from the supplied summary. Write personalised, plain, board-ready lines for a busy marketing team — concise, not an AI essay, no filler.`;
...
=== TCL VOICE — read the exemplars so your prose sounds like the account ===
${tclVoicePromptFragment(6)}

=== TCL BRAND CATALOG (recommendations MUST name brands from this list — never invent, never say "luxury brands" generically) ===
${catalogForPrompt()}
