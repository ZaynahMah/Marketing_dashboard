...
export interface AiPostAnalysis {
  ...
  verdict: string; // Scale Immediately | Repeat | Improve Hook | ...
}
...
/* ------- 20 Brand-Level Content Recommendations (Section 6) ------- */
...
export interface AiBrandRecommendation {
  ...
  concept: string;
  brands: string[]; // MUST be from BRAND_CATALOG
  whyItWillWork: string; // ties to numbers OR trend signal
  expectedKpiImpact: string; // e.g. "Reach +15-20%, Saves +30%"
  audienceIntent: string; // e.g. "Investment dressing consideration"
  trendConnection: string; // named live/cultural trend
  executionNotes: string; // shot list / creative treatment
  difficulty: "Low" | "Medium" | "High";
  predictedPerformance: "Breakout" | "Strong" | "Reliable";
  isFlagshipRecurring: boolean; // 7 of 20 are flagship recurring formats
  flagshipStructure?: string; // when isFlagshipRecurring
}
