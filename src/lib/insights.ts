...
export function strategicInsights(posts: ConsolidatedPost[]): Insight[] {
  ...
  if (buckets.length >= 2) {
    const top = buckets[0];
    const promo = buckets.find((b) => b.bucket === "Sale / Promotion Announcements") ?? buckets[buckets.length - 1];
  ...
  const craft = bucketStats(posts).find((b) => b.bucket === "Craftsmanship / Brand Stories" && b.posts >= 1);
  const sale = bucketStats(posts).find((b) => b.bucket === "Sale / Promotion Announcements" && b.posts >= 1);
  if (craft && (!sale || craft.avgViews >= (sale?.avgViews ?? 0))) {
    insights.push({
      kind: "brand",
      title: "Craft narratives align with positioning",
      body: `Craftsmanship / Brand Stories content averages ${fmtCompact(craft.avgViews)} views${
        sale ? ` vs ${fmtCompact(sale.avgViews)} for Sale / Promotion content` : ""
...
export function recommendations(posts: ConsolidatedPost[]): Recommendation[] {
  ...
  const craft = buckets.find((b) => b.bucket === "Craftsmanship / Brand Stories");
  if (craft) {
    recs.push({
      headline: "Commission a recurring behind-the-maison craft series",
      rationale: `Craftsmanship / Brand Stories content averages ${fmtCompact(craft.avgViews)} views and ${fmtInt(
        craft.avgSaves
