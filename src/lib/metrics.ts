...
/**
 * Auto-detect the correct report level from the date range of the uploaded data.
 *
 * Monthly  — the data spans ≥ 20 days within a single calendar month, or spans
 *            across month boundaries with ≥ 28 days total.
 * Weekly   — the data spans ≥ 5 days (crosses at least 2 ISO weeks).
 * Daily    — anything shorter (a few days).
 */
export function detectReportLevel(posts: ConsolidatedPost[]): "daily" | "weekly" | "monthly" {
  const dates = posts
    .map((p) => p.publishTime)
    .filter((d): d is string => !!d)
    .map((d) => new Date(d))
  ...
  const spanDays = Math.round((latest.getTime() - earliest.getTime()) / 86400000) + 1;

  // Monthly: ≥ 20 days within the same month, or ≥ 28 days across months.
  const sameMonth = earliest.getFullYear() === latest.getFullYear() && earliest.getMonth() === latest.getMonth();
  if (sameMonth && spanDays >= 20) return "monthly";
  if (spanDays >= 28) return "monthly";

  // Weekly: ≥ 5 days.
  if (spanDays >= 5) return "weekly";
