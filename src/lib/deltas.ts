...
export function kpiDeltas(cur: ViewTotals, prev: ViewTotals | null): KpiDelta[] {
  ...
  return [
    ...
    { key: "impressions", label: "Impressions", current: cur.impressions, previous: p?.impressions ?? null, changePct: pctChange(cur.impressions, p?.impressions ?? null), format: "compact" },
    ...
    { key: "saves", label: "Saves", current: cur.saves, previous: p?.saves ?? null, changePct: pctChange(cur.saves, p?.saves ?? null), format: "int" },
    { key: "shares", label: "Shares", current: cur.shares, previous: p?.shares ?? null, changePct: pctChange(cur.shares, p?.shares ?? null), format: "int" },
    { key: "spend", label: "Spend", current: cur.spend, previous: p?.spend ?? null, changePct: pctChange(cur.spend, p?.spend ?? null), format: "currency" },
    { key: "follows", label: "Followers Gained", current: cur.follows, previous: p?.follows ?? null, changePct: pctChange(cur.follows, p?.follows ?? null), format: "int" },
    ...
    { key: "cpf", label: "CPF", current: cur.cpf, previous: p?.cpf ?? null, changePct: pctChange(cur.cpf, p?.cpf ?? null), lowerIsBetter: true, format: "cost" },
  ];
}

/** For every calendar month present in the view, emit a KpiSnapshot. */
export function monthlyKpiSnapshots(posts: ViewPost[]): { monthKey: string; monthLabel: string; totals: ViewTotals; posts: ViewPost[] }[] {
  const map = new Map<string, ViewPost[]>();
  ...
  return Array.from(map.entries())
    ...
    .map(([k, ps]) => ({ monthKey: k, monthLabel: monthLabel(k), totals: viewTotals(ps), posts: ps }));
}
