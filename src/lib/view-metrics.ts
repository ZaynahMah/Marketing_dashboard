...
export interface ViewTotals {
  ...
  cpv: number | null;
  cpe: number | null;
  cpf: number | null; // Cost per Follower gained
  ctr: number | null;
...
export function viewTotals(posts: ViewPost[]): ViewTotals {
  ...
  return {
    ...
    cpf: (() => {
      const f = s(posts.map((p) => p.follows));
      return f ? spend / f : null;
    })(),
...
export type RankBy = "cpr" | "cpe" | "cpv" | "cpf" | "er" | "views" | "reach" | "follows" | "saves" | "shares";
...
export function rankPosts(posts: ViewPost[], by: RankBy, limit = 5): ViewPost[] {
  ...
  const spend = (p: ViewPost) => nz(p.spend);
  switch (by) {
    case "cpr":
      return arr.filter((p) => p.cpr != null && p.cpr > 0).sort((a, b) => (a.cpr! - b.cpr!)).slice(0, limit);
    case "cpe":
      return arr.filter((p) => p.cpe != null && p.cpe > 0).sort((a, b) => (a.cpe! - b.cpe!)).slice(0, limit);
    case "cpv":
      return arr.filter((p) => p.cpv != null && p.cpv > 0).sort((a, b) => (a.cpv! - b.cpv!)).slice(0, limit);
    case "cpf":
      return arr
        .filter((p) => spend(p) > 0 && nz(p.follows) > 0)
        .map((p) => ({ p, cpf: spend(p) / nz(p.follows) }))
    ...
    case "saves":
      return arr.sort((a, b) => (b.saves ?? 0) - (a.saves ?? 0)).slice(0, limit);
    case "shares":
      return arr.sort((a, b) => (b.shares ?? 0) - (a.shares ?? 0)).slice(0, limit);
  }
}

/** CPF for a single post — nullable when spend=0 or follows=0. */
export function postCpf(p: ViewPost): number | null {
  const s = p.spend ?? 0;
  const f = p.follows ?? 0;
  if (!s || !f) return null;
  return s / f;
