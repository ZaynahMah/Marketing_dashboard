...
import type { AiResponse, AiStatus, AiSummary } from "@/lib/ai/types";
...
export async function POST(req: NextRequest) {
  ...
  let payload: { summary?: AiSummary; model?: string };
  try {
  ...
  const prompt = buildStrategyPrompt(payload.summary, TCL_BRIEF_CONTEXT);

  try {
    const report = await callGemini(prompt, model, key);
