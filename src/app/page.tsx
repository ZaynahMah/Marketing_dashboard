...
export default function Page() {
  ...
  // Accumulated union of every uploaded post (dedup by shortcode) — this is what
  // powers weekly/monthly "learning over time" across multiple daily uploads.
  const allPosts: ConsolidatedPost[] = useMemo(() => {
    const seen = new Map<string, ConsolidatedPost>();
    for (const snap of history) for (const p of snap.posts) if (!seen.has(p.shortcode)) seen.set(p.shortcode, p);
    return [...seen.values()];
  }, [history]);
  ...
  const viewPosts = reportType === "daily" ? active?.posts ?? [] : allPosts;
  const providerSnapId = reportType === "daily" ? active?.id ?? "none" : `accumulated-${allPosts.length}`;
    ...
    <div className="min-h-screen">
      <header className="sticky top-0 z-20 border-b border-line bg-paper/90 backdrop-blur">
        <div className="mx-auto flex max-w-shell items-center gap-4 px-6 py-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            {history.length > 0 && reportType === "daily" && (
              <select
                value={activeId ?? ""}
                onChange={(e) => {
                ...
                className="tabular max-w-[180px] rounded-full border border-line bg-surface px-3 py-1.5 text-[12px] text-ink focus:border-graphite"
              >
                {history.map((h) => (
      ...
      <main className="mx-auto max-w-shell px-6 py-8">
        {mode === "upload" || !active ? (
          ...
          <AiProvider
            snapshotId={providerSnapId}
            posts={viewPosts}
            prev={reportType === "daily" ? prev : undefined}
            reportType={reportType}
          >
            <div className="mb-6">
              <ReportDownloads posts={viewPosts} prev={prev} />
            </div>
            {reportType === "weekly" ? (
              <WeeklyReport posts={viewPosts} prev={prev} />
            ) : reportType === "monthly" ? (
              <MonthlyReport posts={viewPosts} />
            ) : layer === "executive" ? (
              <ExecutiveAudit posts={viewPosts} label={active.label} />
            ) : (
              <PerformanceDashboard posts={viewPosts} prev={prev} />
            )}
          </AiProvider>
        )}
        ...
        {mode === "view" && history.length > 1 && (
          <div className="mx-auto mt-6 max-w-2xl">
            <Eyebrow className="mb-2">History · {history.length} uploads · {allPosts.length} posts accumulated</Eyebrow>
            <div className="flex flex-wrap gap-2">
              {history.map((h) => (
                ...
                >
                  {h.label}
                  <button onClick={() => remove(h.id)} className="text-mist hover:text-negative" title="Delete">✕</button>
                </span>
              ))}
            </div>
          </div>
