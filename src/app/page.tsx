...
import { ReportDownloads } from "@/components/reports/ReportViews";
...
import { detectReportLevel } from "@/lib/metrics";
import { Eyebrow } from "@/components/ui";

type Layer = "executive" | "performance";

const VIEW_LABELS: Record<AiReportType, string> = {
  daily: "Daily View",
  weekly: "Weekly View",
...
export default function Page() {
  ...
  // Auto-detect report level from the uploaded data — used ONLY as a heading label.
  const reportType: AiReportType = useMemo(() => {
    if (!active?.posts?.length) return "daily";
    return detectReportLevel(active.posts);
  }, [active]);
    ...
    <div className="min-h-screen">
      <header className="sticky top-0 z-20 border-b border-line bg-paper/90 backdrop-blur">
        <div className="mx-auto flex max-w-shell items-center gap-4 px-6 py-3">
          ...
          {/* Auto-detected view label — just a heading, doesn't change the page structure */}
          {mode === "view" && active && (
            <span className="eyebrow ml-2 hidden rounded-full border border-line bg-surface px-3.5 py-1.5 text-ink lg:inline">
              {VIEW_LABELS[reportType]}
            </span>
          )}
        ...
        {/* Executive / Performance toggle — always visible when data is loaded */}
        {mode === "view" && active && (
          <div className="flex items-center gap-2 border-t border-hairline px-6 py-2">
            <span className="eyebrow text-mist lg:hidden">{VIEW_LABELS[reportType]}</span>
      ...
      <main className="mx-auto max-w-shell px-6 py-8">
        {mode === "upload" || !active ? (
          ...
          <AiProvider snapshotId={providerSnapId} posts={posts} prev={undefined} reportType={reportType}>
            ...
            {/* Same layout for all view types — Executive Audit or Performance */}
            {layer === "executive" ? (
              <ExecutiveAudit posts={posts} label={active.label} archive={archive} />
            ) : (
              <PerformanceDashboard posts={posts} archive={archive} snapshotId={providerSnapId} />
            )}
          </AiProvider>
        )}
